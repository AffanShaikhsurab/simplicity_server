...
def call_contract():
    ...
    required_fields = ['sender', 'contract_name', 'digital_signature', 'public_key', 'timestamp']
    ...
    index, error = blockchain.execute_contract(
        transaction, values['public_key'], values['digital_signature']
    )

    # Ensure `index` is serializable
    if index is not None:
        if hasattr(index, "__float__"):  # Check if it can be converted to a float
            index = float(index)
        elif hasattr(index, "__int__"):  # Check if it can be converted to an int
            index = int(index)
        elif isinstance(index, (list, dict)):  # Allow lists and dicts
            pass
        else:
            index = str(index)  # Fallback: convert to string
...
def new_contract():
    ...
    if index is not None:
        response = {'message': f'Contract will be deployed in Block {index}'}
    else:
        response = {'message': error}
    return flask.jsonify(response), 201
...
def shutdown_session(exception=None):
    ...
    database = BlockchainDb()
    contractDb = ContractStorageDb()
    database.save_blockchain(blockchain)
    database.save_to_firebase()
    contractDb.save_storage_state(blockchain.storage)
