...
class BlockchainDb:
    ...
    def save_to_temp_file(self):
        """
        Save the blockchain to a temporary file. If the directory or file doesn't exist, it creates them.
        """
        try:
            # Define the path to the directory and file
            directory = './temp'
            local_file_path = os.path.join(directory, 'blockchain.json')
            ...
            if not os.path.exists(local_file_path):
                # If the file does not exist, create it with an empty blockchain (or any default content)
                with open(local_file_path, 'w') as f:
                    json.dump({}, f)  # Assuming an empty blockchain at the start
                logger.info(f"File created: {local_file_path}")
            ...
            with tempfile.NamedTemporaryFile(mode='w+', delete=False) as temp_file:
                ...
                # Log the file path and success message
                logger.info(f"Blockchain saved to temporary file: {temp_file.name}")
                self.temp_file = temp_file.name  # Save the temp file path to the instance
            ...
            logger.error(f"An unexpected error occurred while saving blockchain to temporary file: {e}")
    ...
    def revert_to_temp_file(self):
        ...
        if self.temp_file and os.path.exists(self.temp_file):
            try:
                with open(self.temp_file, 'r') as temp_f:
                ...
                print(f"Blockchain reverted to the state in temporary file: {self.temp_file}")
