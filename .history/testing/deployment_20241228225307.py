...
# Create transaction
transaction_payload = create_transaction(
    sender_public_key=wallet.public_key.toCompressed(),
    recipient="0",  # Example recipient for the smart contract
    amount=0,
    contract_code=contract_code,
)
...
try:
    ...
    transaction_payload["public_key"] = wallet.public_key.toCompressed()
    transaction_payload["digital_signature"] = digital_signature

    # Log the signed transaction
    logger.info("Transaction JSON:")
    logger.info(json.dumps(transaction_payload, indent=4, sort_keys=True))
    logger.info(f"Digital Signature (Base64): {digital_signature}")

    # Send the transaction
    endpoint = "http://127.0.0.1:5001/contracts/new"
    send_transaction(transaction_payload, endpoint)
