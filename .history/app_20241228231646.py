...
def call_contract():
    values = flask.request.get_json()
    required = ['sender' , 'contract_name', 'digital_signature', 'public_key']
    if not all(k in values for k in required):
        return 'Missing values', 400
    ...
    index, error = blockchain.execute_contract(transaction, values['public_key'], values['digital_signature'])
    if index is not None:
       response = {'message': f'Contract will be called Block {index}'}
    else:
        response = {'message': error}
    return flask.jsonify(response), 201
