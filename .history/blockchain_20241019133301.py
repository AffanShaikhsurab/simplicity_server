...
class Blockchain:
    ...
    def register_node(self, address, current_address):
        ...
        :return: None
        """
        logging.info(f"Attempting to register node: {address}")
        self.remove_expired_nodes()

        try:
            parsed_url = self._parse_url(address)
            current_url = self._parse_url(current_address)

            if parsed_url.netloc in self.nodes:
                logging.info(f"Node {parsed_url.netloc} already registered")
                return

            self.nodes.add(parsed_url.netloc)
            logging.info(f"Added new node: {parsed_url.netloc}")
            ...
            with concurrent.futures.ThreadPoolExecutor(max_workers=3) as executor:
                futures = [
                    executor.submit(self._update_chain, base_url, current_url.netloc),
                    executor.submit(self._update_nodes, base_url),
                    executor.submit(self._update_ttl, base_url, current_url.netloc) if self.ttl else None
                ]
                concurrent.futures.wait(futures)

            for future in futures:
                if future and future.exception():
                    logging.error(f"Error in async operation: {future.exception()}")

        except ValueError as e:
            logging.error(f"Error parsing URL: {e}")
        except Exception as e:
            logging.error(f"Unexpected error in register_node: {e}")

    def _parse_url(self, url):
        parsed_url = urlparse(url)
        if not parsed_url.netloc:
            raise ValueError(f"Invalid address: {url}")
        return parsed_url
    ...
    def _update_chain(self, base_url, current_netloc):
        try:
            ...
            logging.info(f"Updated chain status: {response.status_code}")
        except requests.RequestException as e:
            logging.error(f"Error updating chain for {base_url}: {e}")
    ...
    def _update_nodes(self, base_url):
        try:
            ...
            response.raise_for_status()
            logging.info(f"Updated nodes for {base_url}")
        except requests.RequestException as e:
            logging.error(f"Error updating nodes for {base_url}: {e}")
    ...
    def _update_ttl(self, base_url, current_netloc):
        ...
        try:
            ...
            response.raise_for_status()
            logging.info(f"Updated TTL for {base_url}")
        except requests.RequestException as e:
            logging.error(f"Error updating TTL for {base_url}: {e}")
