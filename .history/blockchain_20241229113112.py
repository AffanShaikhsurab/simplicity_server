...
class Blockchain:
    ...
    def retrieve_contract_code(self, contract_name):
        for block in self.chain:
            for tx in block['transactions']:
                if 'contract_name' in tx['transaction'] and tx['transaction']['contract_name'] == contract_name and 'code' in tx['transaction'] :
    ...
    def execute_contract(self , transaction ,  public_address, digital_signature):
        ...
        try:
            contract_code = self.retrieve_contract_code(transaction['contract_name'])
            ...
            if transaction["symbol_table"] != None:
                contract_symbol_table = transaction["symbol_table"]
            else:   
            ...
            # print("adding the contract to the symbol table")
            # interpreter.setVariable(
            #     contract_name , 
            #     ast.node.statements.pop(0)
            #     [contract_symbol_table]
            # )
            interpreter = Inter.Interpreter()
            context = Inter.Context("<contract>")
            context.symbol_table = contract_symbol_table
            interpreter.visit(ast.node, context , [contract_symbol_table])
            print("callContractNode is " , contract_name , parameters )
            # contract = interpreter.getVariable(contract_name , [contract_symbol_table])
            # print("contract is " , contract)
            print("calling contract node ")
            result  =  interpreter.visit_CallContractNode(
                Pr.CallContractNode(
                    contract_name , parameters 
                )
                , context , [contract_symbol_table])
            # result  = interpreter.visit(ast.node , context , [contract_symbol_table])
            if result.error:
                return  None , result.error.print()
            transaction["symbol_table"] = context.symbol_table
