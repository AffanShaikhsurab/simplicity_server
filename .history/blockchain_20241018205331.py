...
class Blockchain:
    ...
    def register_node(self , address , current_address):
        ...
        * The port number (if specified)
        ...
        print(
            "Sending nodes update request to " + str(parsed_url) + " and " + str(current_url.netloc)
        )
        requests.post(f'http://{parsed_url}/nodes/update_chain' , json=[self.chain , current_url.netloc , list(self.hash_list) , list(self.nodes)])
