...
class Interpreter:
    ...
    def visit_NumberNode(self , node , context , symbol_table  : List[SymbolTable]):
        print("visiting number node" , node.token.value)
        result = InterpreterResult().success(Number(int( str(node.token.value ))).set_context(context).set_pos(node.pos_start , node.pos_end))
