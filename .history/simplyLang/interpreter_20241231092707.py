...
class Interpreter:
    ...
    def visit_VariableAccessNode(self , node : Pr.VariableNode, context , symbol_table  : List[SymbolTable], ):
        variable_name = node.variable_name.value
        value = None
        value = self.getVariable(variable_name , symbol_table)
        
        print("Variable value is " , variable_name ,  value )
        if not value:
            if node.isSave:
                self.setVariable(variable_name , value  , symbol_table[0])
        ...
        try:
            ...
            value = value
        print("retruning sucess from variable access node" ,value) 
