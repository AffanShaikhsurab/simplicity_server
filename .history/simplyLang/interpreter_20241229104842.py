...
class Interpreter:
    ...
    if len(contract[1]) != len(node.parameters):
        return res.failure(Lex.InvalidSyntaxError(
            f"Invalid number of parameters in Contract '{str(node.contract_name)}'",
        ))
    ...
    for i in range(len(contract[1])):
        ...
        if value:
            ...
            contract_symbol_table.set(str(contract[1][i]), value)  # Convert key to str if necessary
        else:
            contract_symbol_table.set(str(contract[1][i]), node.parameters[i])

    if contract_symbol_table is not None:
        symbol_tables = symbol_table + [contract_symbol_table]  # Ensure both are lists

    for i in contract[0]:
        value = res.register(self.visit(i, context, symbol_tables))
        if res.error:
            return res.failure(Lex.InvalidSyntaxError(
                f"Error processing contract: {str(res.error)}"
            ))  # Ensure res.error is converted to a string if needed
