...
class Interpreter:
    ...
    def visit_VariableAccessNode(self, node: Pr.VariableNode, context, symbol_table: List[SymbolTable]) -> InterpreterResult:
        """
        ...
        if value is None:
            ...
            print("isSave" , str(isSave))
            if isSave == True:
                print(f"Variable '{variable_name}' is undefined. Saving default value.")
                self.setVariable(variable_name, None, symbol_table)
                return InterpreterResult().success(None)
        ...
        try:
            ...
            # If `value` does not support `copy()` or `set_pos`
            print(f"Value '{variable_name}' does not support copying or position setting.")
