...
class Number:
    ...
    def add(self , other):
        ...
        print("type(self)" , type(self.value) , self.value)
        
        if isinstance(other , Number):
            return Number(int(str(self.value)) + other.value).set_context(self.context).set_pos(self.pos_start , other.pos_end), None
...
class Interpreter:
    ...
    def visit_NumberNode(self , node , context , symbol_table  : List[SymbolTable]):
        print("value of token is " , node.token.value)
        value_str = str(node.token.value)

        try:
            # Attempt to parse the value as a float first
            value = float(value_str)
            # Convert to int if it's a whole number
            value = int(value) if value.is_integer() else value
        except ValueError:
            # Handle the case where value is not a valid number
            raise ValueError(f"Invalid token value: '{value_str}' is not a number.")
    ...
    def visit_VariableAccessNode(self, node: Pr.VariableAccessNode, context, symbol_table: List[SymbolTable]) -> InterpreterResult:
        """
        Handles variable access in the interpreter.

        Parameters:
        - node: The VariableNode to be visited.
        - context: The current context in the interpreter.
        - symbol_table: List of symbol tables to look up variables.

        Returns:
        - InterpreterResult: Result object indicating success or failure.
        """
        variable_name = node.variable_name.value
        print(f"Accessing variable '{variable_name}'")
        # Retrieve the variable's value from the symbol table
        value = self.getVariable(variable_name, symbol_table)
        print("value getting is " , value)
        if value is None:
            print("the variable value is none...")
            print("the variable node is " , node )

            print(f"Variable '{variable_name}' is undefined. Saving default value.")
            self.setVariable(variable_name, 0 , symbol_table)
            return InterpreterResult().success(0)

            # error_message = f"Variable '{variable_name}' is not defined."
            # print(error_message)
            # return InterpreterResult().failure(
            #     Lex.InvalidSyntaxError(error_message, node.pos_start, node.pos_end)
            # )

        print(f"Returning success from VariableAccessNode: {value}")
    ...
    def visit_VariableNode(self , node : Pr.VariableNode, context , symbol_table  : List[SymbolTable]):
        ...
        if node.isSave:
                        # Always store in global scope (first symbol table)
            symbol_table[0].set(var_name, value)
        else:
    ...
    def setVariable(self , variable , value , symbol_table):
        ...
        try:
            ...
            symbol_table[0].set(variable , value)
    ...
    def visit_ContractNode(self, node: Pr.ContractNode, context, symbol_table):
        res = InterpreterResult()
        #here node.variables contain variable names ( eg: dog ) and not the value of the variable
        print("saving the contract  " , node.contract_name.value)
        self.setVariable(node.contract_name.value,  [node.body , node.variables] , symbol_table)
        return res.success(None)

    def visit_CallContractNode(self, node: Pr.CallContractNode , function_name ,  context, symbol_table):
        res = InterpreterResult()
        contract = self.getVariable(node.contract_name , symbol_table)
        print("getting contract  " , contract)
        if not contract:
        ...
        print("looping..")
        print(
            "paramters passed are" , node.parameters
        )
        ...
        for i in contract[0]:
            print("visiting " , i)
            print(type(i))
            print("the funtion is " , function_name)
            if isinstance(i , Pr.FunctionNode):
                print("this is fcuntion " , i.function_name.value == function_name)
                if i.function_name.value == function_name:
                    self.function_list.append( i)
                    print("fucntion added to funtion list ")
                    function_node = Pr.FunctionCallNode(
                        i.function_name.value,
                        i.body,
                        node.parameters
                    )
                    print("fucntion node created ")
                    value = res.register( self.visit_FunctionCallNode(function_node , context , symbol_tables) )
                    print("value returned from visit " , value)
                    
                    if not value :
                        print("returning error" , res.error.print())
                        return res.error   
                    else:
                        return res.success(value)
        return res.error(
            Lex.InvalidSyntaxError(
                f"Function {function_name} is not defined in Contract {node.contract_name}"
            )
        )
    ...
    def visit_ReturnExprNode(self, node : Pr.ReturnExprNode, context , symbol_table  : List[SymbolTable]):
        res = InterpreterResult()
        token = node.token
        print("type of return node "  , type(token))
        if type(token) == str:
            return res.success(token)
    ...
    def visit_FunctionCallNode(self, node : Pr.FunctionCallNode, context , symbol_table : List[SymbolTable] ):
            ...
            if func.function_name.value == node.function_name:
                ...
                if func.variables != None:
                    ...
                    for i in func.variables:
                        print("seeing if vairbale is present " , i)
                        print(
                            node.parameters
                        )
                        if i not in node.parameters:
                            print(
                                
                                f"'Parameter {i} not passed in function {node.function_name}   '",
                       
                            )
                            
                            return InterpreterResult().failure(Lex.InvalidSyntaxError(
                                f"'Invalid  parameter {i} are passed in function {node.function_name}   '",
                                node.pos_start,
                                node.pos_end
                            ))
                            
                    print("looping")
                    # Ensure node.parameters is a list of tuples or key-value pairs
                    for i, (key, value) in enumerate(node.parameters):
                        print("setting value", value)

                        # Retrieve the value from the symbol table
                        retrieved_value = self.getVariable(key, symbol_table)

                        # Set the value in the function symbol table
                        if retrieved_value:
                            function_symbol_table.set(key, retrieved_value)
                        else:
                            function_symbol_table.set(key, value)
        ...
        for expr in function:
            ...
            if res.error:
                return res
            print("the value is of expr " ,expr ,   value)
    ...
    #returns tuple value for paramter handling
    def get_value(parameters, key):
    ...
    def visit_TransferNode(self, node: Pr.TransferNode, context, symbol_table):
        res = InterpreterResult()

        # Directly use the values provided by the TransferNode
        transfer_details = {
            "self_address": self.address,  # Assuming this is an instance attribute
            "node_address": node.address,  # Address provided in the node
            "node_amount": node.amount,    # Amount provided in the node
        }

        # Return the details as a success result
        return res.success(transfer_details)
