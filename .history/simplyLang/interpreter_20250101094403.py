...
class Interpreter:
    ...
    def visit_TransferNode(self, node: Pr.TransferNode, context, symbol_table):
        res = InterpreterResult()
        print("sending money" , node.amount)
        amount = self.getVariable(node.amount , symbol_table)
        print("amount is " , amount)
        result , error = self.blockchain.send_money(self.address, node.address, amount)
        print("value returning is " , result)
        if not result:
            return res.failure(Lex.InvalidSyntaxError(
                f"Transfer failed {error} ", node.pos_start, node.pos_end
            ))
