...
def register_nodes():
    ...
    for node in nodes:
        print("this is parent node","https://simplicity-server.onrender.com")
        blockchain.register_node(node,"https://simplicity-server.onrender.com")
...
def call_contract():
    ...
    required_fields = ['sender', 'contract_address', 'digital_signature', 'public_key', 'timestamp' , 'function']
...
def update_storage():
    ...
    try:
        # Convert the serialized state back to a usable format
        deserialized_state = {}
        for contract_name, data in storage_state.items():
            deserialized_state[contract_name] = {
                "contract_name": data["contract_name"],
                "symbol_table": blockchain.storage_db._deserialize_symbol_table(data["symbol_table"])
            }
        
        blockchain.storage = deserialized_state
        blockchain.storage_db.save_storage_state(deserialized_state)
        
        return 'Storage state updated successfully', 200
    except Exception as e:
        print(f"Error processing storage state update: {e}")
        import traceback
        traceback.print_exc()
        return f'Error processing storage state: {str(e)}', 500
