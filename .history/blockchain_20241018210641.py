...
class Blockchain:
    ...
    def register_node(self, address, current_address):
        ...
        try:
            ...
            try:
                print(
                    f"Added new node: {parsed_url.netloc}",
                    f"{scheme}://{parsed_url.netloc}/nodes/update_chain"
                )
                requests.post(f'vip-streets-occur-thing.trycloudflare.com/nodes/update_chain', 
                              json=[self.chain, current_url.netloc, list(self.hash_list), list(self.nodes)],
                              timeout=5)
                
                requests.post(f'vip-streets-occur-thing.trycloudflare.com/nodes/update_nodes', 
                              json={"nodes": list(self.nodes)},
                              timeout=5)
