...
class Blockchain:
    ...
    def execute_contract(self , transaction ,  public_address, digital_signature):
        ...
        try:
            ...
            contract_transaction = SmartContractTransaction(public_address , contract_name , parameters ,public_address , digital_signature)
            print("contract transaction is " , contract_transaction)
            self.current_transactions.append({
                        "transaction": contract_transaction.to_dict(),
                        "public_address": public_address,
                        "digital_signature": digital_signature
                    })
            print("stroring the state")
            #calcuting the cost of the transaction for saving the state
            cost = self.storage_db.calculate_storage_cost(context.symbol_table)
            print("calculating the balance ")
            balance = self.calculate_address_balance(public_address)
            print("checking balance " , balance)

            if cost > balance:
                return None , "Insufficient balance to cover storage cost."
            result , error= self.send_money(
                public_address ,
                self.public_key
            )
            if not result:
                None , "Insufficient balance to cover storage cost."           
    ...
    def calculate_address_balance(self , public_address):
        sender_balance = 0
        for block in self.chain:
            for transaction in block['transactions']:
                if "recipient" in transaction["transaction"]:
                    if transaction['transaction']['recipient'] == public_address:
                        sender_balance += transaction['transaction']['amount']
                        print(f"Sender balance: {sender_balance}")
                if "sender" in transaction['transaction']:
                    if transaction['transaction']['sender'] == public_address:
                        sender_balance -= transaction['transaction']['amount']
                        print(f"Sender balance: {sender_balance}")
        ...
        for tx in self.current_transactions:
            if "recipient" in transaction["transaction"]:
                ...
                if tx['transaction']['recipient'] == public_address:
                    sender_balance += tx['amount']
            if "sender" in transaction['transaction']:
                if tx['transaction']['sender'] == public_address:
                    sender_balance -= tx['transaction']['amount'] 
                 
        print(f"Sender balance: {sender_balance}")
        if  sender_balance >= public_address:
            return True ,None
        else:
            self.error =  "Transaction will not be added to Block due to insufficient funds"        
            return False , self.error
