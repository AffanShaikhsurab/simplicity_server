...
class Blockchain:
    ...
    def execute_contract(self , transaction ,  public_address, digital_signature):
        ...
        try:
            ...
            parameters = []
            if transaction.get('parameters') :
                for i in range(len( transaction['parameters'])):
                    parameters.append(
                        transaction["parameters"][i]
                    )
                    contract_symbol_table.set(str(i) , transaction['parameters'][i])
            print("callContractNode is " , Pr.CallContractNode(
