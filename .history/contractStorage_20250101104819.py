...
class ContractStorageDb:
    ...
    def _deserialize_symbol_table(self, serialized_table):
        ...
        return symbol_table
    def calculate_storage_cost(self, data):
        """
        Calculate storage cost based on data size.
        Base rate: 0.0001 tokens per byte
        Minimum cost: 0.001 tokens
        """
        try:
            print(
                f"Calculating storage cost for data: {data}"
            )

            # Serialize data using a helper function (if needed)
            serialized_data = self._serialize_symbol_table(data)
            
            # Ensure the serialized data is in string format
            if isinstance(serialized_data, dict) or isinstance(serialized_data, list):
                json_data = json.dumps(serialized_data)
            else:
                json_data = str(serialized_data)

            # Calculate size in bytes
            byte_size = len(json_data.encode('utf-8'))
            
            # Base rate per byte (0.0001 tokens)
            BASE_RATE = 0.0001
            ...
            # Calculate cost
            cost = byte_size * BASE_RATE
            
            # Apply minimum cost if calculated cost is too low
            final_cost = max(cost, MIN_COST)
            print(
                f"Byte size: {byte_size} bytes"
                f"Base rate: {BASE_RATE} tokens per byte"
                f"Minimum cost: {MIN_COST} tokens"
                f"Cost: {cost} tokens"
                f"Final cost: {final_cost} tokens"
            )
            return {
                'byte_size': byte_size,
                'base_rate': BASE_RATE,
            ...
            print(f"Error calculating storage cost: {e}")
            return None
    ...
    def broadcast_storage_state(self, nodes, storage_state):
        ...
        # Serialize the storage state before broadcasting
        try:
            serialized_state = {}
            for contract_name, data in storage_state.items():
                serialized_state[contract_name] = {
                    "contract_name": data["contract_name"],
                    "symbol_table": self._serialize_symbol_table(data["symbol_table"])
                }
            ...
            for node in nodes:
                try:
                    ...
                    response = requests.post(
                        ...
                        json={"storage_state": serialized_state}
                    )
            ...
            print(f"Error serializing storage state: {e}")
            import traceback
            traceback.print_exc()
